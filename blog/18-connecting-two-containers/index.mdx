...
---
There's an issue with our implementation in the last post. Have you spotted it?
...
Let's now switch to running our app with Docker Compose also on our production server. First, verify Docker Compose is already installed:
```bash
juraj@server:~$ docker compose version
Docker Compose version v2.12.2
```

We need to pass the port number as an environment variable. Let's update the `ports` option in `docker-compose.yml`:
```yml
services:
  app:
    image: app
...
We can pass environment variable by simply prepending it before the `docker compose up` command:

```bash
SERVER_PORT=8080 docker compose up -d
```

Finally, let's update the start command in `prod_deploy.sh` and deploy!
```bash
