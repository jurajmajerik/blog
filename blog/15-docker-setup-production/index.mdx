...
---
Let's test now use Docker in production. Before we deploy our changes, we need to install Docker on our production server. Here, we'll only install [Docker Engine](https://docs.docker.com/engine/) instead of *Docker Desktop*, as we don't need the GUI.
