...
Whenever a car arrives at a new coordinate within a section, we can rotate it. We only do this from the index `1` of a section onwards (remember that we need two points to determine the rotation). If there is a rotation already running, we wait for it to complete before beginning a new one.
```js
    // ...
