...
Now onto rotation. We always need two points to calculate the rotation of the car - the current coordinate and the previous coordinate. Suppose the initial car position is `[7,20]`. If the next location is `[8, 20]`, we know the car is advancing on the x-axis in the *westerly* direction. Therefore, its azimuth is 90 degrees. Setting the `transform: rotate()` property of the car to 90 degrees will get us the desired rotation.

![car rotation](./img-1.png)

Before we tackle rotating the car, we need to calculate the number of turns a car will make within a section. Remember - a *section* is part of the path a car traverses during a single animation cycle. A section starts at the car's current position on the Map and ends at the latest updated location from the server.
...
*Again, the above methods are tested in* `movement.test.js`. *Take a look at the test cases to better understand inputs and outputs.*
...
Whenever a car arrives at a new coordinate within a section, we might need to rotate it. We only do this from the index `1` of a section onwards (remember that we need two points to determine the rotation).
```js
    // ...
...
```
*Within the loop, I've had to add a couple of if statements to deal with the edge cases when the rotation goes over the target. I don't know where I made a mistake. The increments are calculated such that they should always add up to the target without a remainder. I'll update this section once I debug this.*
