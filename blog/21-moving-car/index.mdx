...
Let's draw this out. For the sake of simplicity, let's show this on a single axis. Let's assume we are polling for an update every second - that's 1000 milliseconds. If we are to display the animation for the entirety of the time until the next update, we need to traverse four points on the map (8 -> 12) in precisely 1,000 milliseconds.
...
In that case, we should stop our current animation and kick off a new animation round. The new animation will calculate a new section to traverse. This new section's start will be the map's current location. That is, the furthest point we got to during our latest animation.

![car svg](./img-2.png)

For completeness, what happens if we complete the animation and still haven't received a new location update? No big deal - the car will just stop moving and wait for the new coordinates.
...
```
I've added unit tests for these methods in `movement.test.js`. The `create-react-app` already has the Jest testing library fully set up. When you run `npm run test`, all files ending with `*.test.js* are being ran in a `watch` mode. The tests are re-run whenever you save your changes, giving us a nice iteration cycle.

Finally, let's put everything together in the `move()` method of the `Car` class. When we receive a next point, we will calculate the upcoming section, and the increment and start updating the coordinates. After calculating a new update, we set a new state of the `Car`, triggering a re-render of the visual. We wait for the duration of the `refreshInterval` between recalculating again.
```js
  async move(next) {
