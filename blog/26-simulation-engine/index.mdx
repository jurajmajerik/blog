...
tags: ['node']
---
Today, we are starting to build the simulation engine. Let's see how it's going to operate.

The simulation engine is going to simulate the behavior of *entities*. An entity can be a driver or a customer. Let's consider the customer entity.

A customer will be modeled with the Customer class. The essence of the simulation is this: every *n* milliseconds, the customer will make a certain decision. The time interval between decisions is the *refresh rate*.

What decision is the customer going to make at a given moment? That depends on *constraints*. For example, if a customer is inactive, they can decide whether to stay inactive or become active.

The decision will be picked randomly from the options, with some probability. Using the same example, if a customer is inactive, they can have a 5 % chance of becoming active. That means, on average, if they are inactive, they will become active after 20 refresh cycles.

There can be multiple decisions at a single decision moment. Once the customer completes the decision-making process and updates its internal state, it will update the database. Then, it will wait for the duration of the *refresh interval*, after which new decisions will be made.
...
And this is how it looks like on the map. The cars are not aware of the customers yet, but we will address that soon!
