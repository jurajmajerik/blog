...
There is on issue in our server code. It currently starts the server ```8080```, which is a local port. You cannot connect to this port from the outside. So if we want to expose our server to the world - i.e. allow anyone to make HTTP requests to it, we need to start the server at port ```80```.
